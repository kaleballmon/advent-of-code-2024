...
defmodule Diagonals do
  def get_main_diagonals(puzzle_graphemes) do
    ...
    Enum.map(Enum.to_list(index_diff_range), fn diff -> get_main_diagonals(puzzle_graphemes, diff) end)
  ...
  def get_anti_diagonals(puzzle_graphemes) do
    ...
    Enum.map(Enum.to_list(index_sum_range), fn index_sum -> get_anti_diagonals(puzzle_graphemes, index_sum) end)
      |> Enum.map(fn row -> Enum.join(row) end)
  end
...
puzzle_lines_top_to_bottom = Enum.map(
  Enum.to_list(0..(length(List.first(puzzle_graphemes)) - 1)),
  fn n -> Enum.map(puzzle_graphemes, fn row -> Enum.at(row, n) end)
end)
 |> Enum.map(fn row -> Enum.join(row) end)
...
total = (puzzle_lines_left_to_right |> count_xmas.()) + (puzzle_lines_top_to_bottom |> count_xmas.()) + (puzzle_lines_main_diagonal |> count_xmas.()) + (puzzle_lines_anti_diagonal |> count_xmas.())
IO.puts(total)
